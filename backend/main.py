...
async def next_checkpoint(request: CheckpointRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"{checkpoint_detail_file} not found. Run the /transcript endpoint first.")
    except json.JSONDecodeError:
        raise HTTPException(status_code=500, detail=f"Error decoding JSON from {checkpoint_detail_file}.")
    except Exception as e:
         raise HTTPException(status_code=500, detail=f"Error reading checkpoint detail file: {str(e)}")
    ...
    try:
        ...
        checkpoint_info_str = json.dumps(checkpoint_info)
         ...
         raise HTTPException(status_code=400, detail=f"Checkpoint index {request.checkpoint_number} out of range in detail file.")
    except KeyError as e:
         raise HTTPException(status_code=500, detail=f"Missing key {e} in checkpoint detail data.")

    prompt_template = checkpoint_prompt
    llm_prompt = prompt_template.replace("{{analysis_summary}}", analysis_summary)
    ...
    try:
        print(f"Sending checkpoint prompt for index {request.checkpoint_number} to LLM...")
        response = client.models.generate_content(
        ...
        print(f"Error calling LLM for checkpoint {request.checkpoint_number}: {e}")
...
async def transcript(request: TranscriptRequest):
    try: 
        ...
        print(f"Transcript processed. Number of checkpoints: {new_num_checkpoints}")
        ...
        current_state["current_checkpoint"] = 0 # Reset checkpoint to 0 when new transcript is processed
